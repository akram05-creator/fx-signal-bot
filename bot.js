...
const [closedToday, stillActive] = await Promise.all([
  dbSelect('trades', `closed_at=gte.${todayISO}T00:00:00Z&closed_at=lte.${todayISO}T23:59:59Z&order=closed_at.asc`),
  dbSelect('trades', `status=eq.active&order=created_at.asc`)
]);
const closedIds = new Set((closedToday||[]).map(t => t.id));
const trades = [
  ...(closedToday || []),
  ...(stillActive || []).filter(t => !closedIds.has(t.id))
];
    ...
    const tradeLines = trades.map((t,i) => {
      ...
      const openDay  = new Date(t.created_at).getUTCDay();
      const todayDay = new Date().getUTCDay();
      const openDayLabel = openDay !== todayDay
        ? ` <i>(trade ${dayNames[openDay]})</i>`
        : '';
     return `  ${i+1}. ${sig} ${t.pair} -> ${result}${openDayLabel}`;
    }).join('\n');
...
// REPLACE calcStructuredSLTP with this version:
...
function calcStructuredSLTP(candles1h, candles15m, candles4h, price, isBuy, dec, atrValue=null) {
  ...
  // Max SL distance (intraday cap)
  const maxSLDist = dec === 2 ? 50.0 : dec === 3 ? 0.30 : 0.0030;
  const minSLDist = dec === 2 ? 8.0  : dec === 3 ? 0.05 : 0.0005;

  // Proximity threshold: OB/Liq valid only if within X of nearest SR zone
  // Gold: 20pts, Forex: 20pips, JPY: 0.20
  const nearThreshold = dec === 2 ? 20.0 : dec === 3 ? 0.20 : 0.0020;

  // SR Channels zones
  const srZones = findSRChannels(candles1h, price, dec, 10, 8, 200);

  // Order Blocks
  const obData = findOrderBlocks(candles1h, price, dec);

  // Liquidity Zones
  const liqData = findLiquidityZones(candles1h, price, dec);

  let sl = null;

  if (isBuy) {
    // BUY: SL ta7t nearest support zone
    // OB + Liq = valid ghir ila 9rib mn support zone (< nearThreshold)
    const supportZones = srZones
      .filter(z => z.hi < price)
      .sort((a,b) => b.hi - a.hi); // nearest first

    if (supportZones.length > 0) {
      const nearestSup = supportZones[0];
      const candidates = [nearestSup.lo]; // toujours: ta7t support zone lo

      // Bull OB bottom: valid ghir ila 9rib mn support zone
      if (obData.bullOB && obData.bullOB.bottom < price) {
        const dist = Math.abs(obData.bullOB.bottom - nearestSup.lo);
        if (dist < nearThreshold) candidates.push(obData.bullOB.bottom);
      }

      // Sellside liq bottom: valid ghir ila 9rib mn support zone
      if (liqData.sellsideLiq && liqData.sellsideLiq.bottom < price) {
        const dist = Math.abs(liqData.sellsideLiq.bottom - nearestSup.lo);
        if (dist < nearThreshold) candidates.push(liqData.sellsideLiq.bottom);
      }

      // SL = ta7t min(candidates) - buffer
      const lowestLevel = Math.min(...candidates);
      sl = parseFloat((lowestLevel - pip * 3).toFixed(dec));
    }

  } else {
    // SELL: SL fo9 nearest resistance zone
    // OB + Liq = valid ghir ila 9rib mn resistance zone (< nearThreshold)
    const resistanceZones = srZones
      .filter(z => z.lo > price)
      .sort((a,b) => a.lo - b.lo); // nearest first

    if (resistanceZones.length > 0) {
      const nearestRes = resistanceZones[0];
      const candidates = [nearestRes.hi]; // toujours: fo9 resistance zone hi

      // Bear OB top: valid ghir ila 9rib mn resistance zone
      if (obData.bearOB && obData.bearOB.top > price) {
        const dist = Math.abs(obData.bearOB.top - nearestRes.hi);
        if (dist < nearThreshold) candidates.push(obData.bearOB.top);
      }

      // Buyside liq top: valid ghir ila 9rib mn resistance zone
      if (liqData.buysideLiq && liqData.buysideLiq.top > price) {
        const dist = Math.abs(liqData.buysideLiq.top - nearestRes.hi);
        if (dist < nearThreshold) candidates.push(liqData.buysideLiq.top);
      }

      // SL = fo9 max(candidates) + buffer
      const highestLevel = Math.max(...candidates);
      sl = parseFloat((highestLevel + pip * 3).toFixed(dec));
    }
  }

  // Fallback: 30m swing if no SR zone found
  if (!sl) {
    const c30 = candles15m.slice(-30);
    const swH = c30.length ? Math.max(...c30.map(x=>x.h)) : price;
    const swL = c30.length ? Math.min(...c30.map(x=>x.l)) : price;
    sl = isBuy ? swL - pip * 3 : swH + pip * 3;
  }

  // Validate SL distance
  let slDist = Math.abs(price - sl);

  // Zone b3ida bzaf (> 2x max) = signal invalid → return null
  if (slDist > maxSLDist * 2) return null;
...
async function runScan() {
  ...
  // Max 1 trade at risk (tp1_hit = false means SL not yet moved to BE)
  const allActiveTrades = await dbSelect('trades', 'status=eq.active&limit=10');
  const atRiskCount = (allActiveTrades || []).filter(t => !t.tp1_hit).length;
  if (atRiskCount >= 1) {
    log(`-> Max 1 trade at risk atteint (${atRiskCount} trade(s) still at risk) - scan bloqué`);
    return;
  }
  ...
  log(`[SCAN] Scanning ${candidates.length} pairs: ${candidates.map(p => p.label+'('+p.tech.totalScore+')').join(' | ')} (at risk: ${atRiskCount}/1)`);

  // Scan each candidate — AI decides for each
  for (const best of candidates) {
    // Skip if a trade at risk already exists
    const currentActive = await dbSelect('trades', 'status=eq.active&limit=10');
    const currentAtRisk = (currentActive || []).filter(t => !t.tp1_hit).length;
    if (currentAtRisk >= 1) {
      log('-> Max 1 trade at risk reached - stopping scan');
      break;
    }
...
Reply ONLY in raw JSON no markdown:
{
  ...
  try {
    ...
    if(!aiStructured){
  log(`-> Signal cancelled: SL zone too far [${best.label}]`);
  continue;
}
